...
module.exports = class {
    constructor() {
        this.db = sequelize
        this.model = this.db.model('MessageType')
    }
    async add(messageTypeEntity) {
        const err = []
        const { Type } = messageTypeEntity
        if (_.isUndefined(Type) || _.isNull(Type)) err.push("Type is required in field 'Type'")
 
        if (err.length > 0) return err
        else {
            return await this.model.create({ Type }, { raw: true })
        }
